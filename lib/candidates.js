...
import { getConfig, setConfig, addWorkerLog, dataDir, touchWorkerHeartbeat } from './db.js';
import {
  findGoodDeals,
  getHashrateGH,
  effectiveMinHours,
...
let building = false;
let renting = false;
let lastNoKeysLogAt = 0;
let lastDisabledLogAt = 0;
let lastOverlapLogAt = 0;

function logThrottled(key, action, details, minGapMs = 120000) {
  const now = Date.now();
  if (key === 'nokeys') {
    if (now - lastNoKeysLogAt < minGapMs) return;
    lastNoKeysLogAt = now;
  } else if (key === 'disabled') {
    if (now - lastDisabledLogAt < minGapMs) return;
    lastDisabledLogAt = now;
  } else if (key === 'overlap') {
    if (now - lastOverlapLogAt < minGapMs) return;
    lastOverlapLogAt = now;
  }
  addWorkerLog(action, details);
}
...
export async function buildCandidates(client) {
  if (building) {
    logThrottled(
      'overlap',
      'skip',
      'Candidate build skipped — previous scan still running (MRR calls are serialized; high scan caps feel slow)',
      60000,
    );
  ...
  const t0 = Date.now();
  let scanned = 0;
  touchWorkerHeartbeat({ phase: 'candidates' });
  ...
  try {
    ...
    addWorkerLog(
      'check',
      `Building candidates: algo=${type} funded=${funded.join(',') || 'none'} ` +
        `scan≤${maxScan}/ccy top=${topN} ending=${includeEnding ? `${endingMinutes}m` : 'off'}`,
    );
    ...
    if (!funded.length) {
      snapshot = {
        ...
        stats: {
          ...
          max_scan: maxScan,
          top_n: topN,
          matched_before_top: 0,
        },
      };
      setConfig('candidates_updated_at', snapshot.updated_at);
      setConfig('candidates_count', '0');
      persistSnapshot();
      addWorkerLog('skip', `No funded balances — nothing to scan (${Date.now() - t0}ms)`);
      return getCandidateSnapshot();
    }
    ...
    persistSnapshot();
    touchWorkerHeartbeat({ phase: 'candidates_done' });
    addWorkerLog(
      'check',
      `Candidates ready: ${top.length} kept (of ${all.length}) · scanned ${scanned} · ${snapshot.stats.duration_ms}ms · funded ${funded.join(',')}`,
    );
  } catch (err) {
...
export function startCandidateBuilder(getClient) {
  ...
  const run = async () => {
    touchWorkerHeartbeat({ phase: 'tick' });
    const client = getClient();
    if (!client) {
      logThrottled('nokeys', 'error', 'Candidate tick skipped — API keys not configured');
      return;
    }
...
export function startRentLoop(getClient, rentFn) {
  ...
  const run = async () => {
    touchWorkerHeartbeat({ phase: 'rent_tick' });
    if (renting) {
      logThrottled('overlap', 'skip', 'Rent tick skipped — previous rent decision still running', 60000);
      return;
    }
    if (getConfig('worker_enabled') !== 'true') {
      logThrottled(
        'disabled',
        'skip',
        'Rent loop idle — worker stopped (candidates still refresh). Toggle Start on Auto Rent to rent.',
      );
      return;
    }
    renting = true;
    ...
    try {
      ...
      addWorkerLog('check', `Rent decision finished in ${Date.now() - t0}ms`);
    } catch (err) {
      addWorkerLog('error', `Rent tick failed: ${err.message}`);
    } finally {
      renting = false;
      setConfig(
        'next_check',
