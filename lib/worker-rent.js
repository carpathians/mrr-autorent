...
import { getConfig, setConfig, addWorkerLog, addRentalLog, touchWorkerHeartbeat } from './db.js';
import {
...
async function rentFromCandidates(client) {
  const t0 = Date.now();
  touchWorkerHeartbeat({ phase: 'rent' });
  ...
  if (!snap.candidates?.length || ageMs > 150000) {
    addWorkerLog('check', `Refreshing candidates before rent (snapshot age ${Math.round(ageMs / 1000)}s)`);
  ...
  addWorkerLog(
    'check',
    `Rent check: ${available.length} available, ${ending.length} ending soon ` +
      `(updated ${latest.updated_at || 'never'}) · ${Date.now() - t0}ms so far`
  );
  ...
  try {
    ...
    addWorkerLog(
      'rent',
      `Rented ${pick.name} (${pick.currency}) −${pick.discount}% vs market for ${length}h (bal ${bal}) · API ${Date.now() - rentT0}ms · total ${Date.now() - t0}ms`,
      pick.id,
      rentalId
    );
...
export function startWorker() {
  stopWorker();
  touchWorkerHeartbeat({ phase: 'start' });
  startCandidateBuilder(getClient);
  startRentLoop(getClient, rentFromCandidates);
  addWorkerLog('start', 'Worker process up: candidates every 60s, rent every 3m when enabled');
}
...
export function stopWorker() {
  ...
  stopRentLoop();
  mrrClient = null;
  addWorkerLog('stop', 'Worker process stopped');
}
