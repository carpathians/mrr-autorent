...
import { getConfig, setConfig, addWorkerLog } from './db.js';

const API_BASE = 'https://www.miningrigrentals.com/api/v2';
/** Persist slow MRR calls so Worker Log can explain UI lag */
const SLOW_MS = 2500;
let lastSlowLogAt = 0;
...
export class MRRClient {
  ...
  async _doRequest(method, endpoint, { query = {}, body = null } = {}) {
    ...
    try {
      ...
      // Throttle slow-call noise (at most ~1/15s) but always console.warn
      if (ms >= SLOW_MS) {
        const line = `Slow MRR ${method} ${signEndpoint} ${ms}ms`;
        console.warn(`[mrr] ${line}`);
        if (Date.now() - lastSlowLogAt > 15000) {
          lastSlowLogAt = Date.now();
          try { addWorkerLog('check', line); } catch { /* db may be closed */ }
        }
      }
      ...
      // Network / parse failures — API business errors already throw above
      if (!msg.startsWith('MRR API error')) {
        try {
          addWorkerLog('error', `MRR ${method} ${signEndpoint} failed after ${ms}ms: ${msg}`);
        } catch { /* ignore */ }
      }
      throw err;
    }
